...
export default function VideoBackground({ videos, className = "", children }: VideoBackgroundProps) {
  ...
  return (
    <div ref={containerRef} className={`relative overflow-hidden ${className}`}>
      ...
      <div className={`absolute inset-0 z-10 ${isMobile ? "bg-black bg-opacity-50" : "bg-black bg-opacity-40"}`} />
      ...
      {isMobile && (
        <>
          ...
          {navigator.connection && navigator.connection.saveData && (
            <div className="absolute top-4 right-4 z-30 bg-black bg-opacity-75 text-white px-3 py-1 rounded-full text-xs">
              Data Saver Mode
            </div>
          )}
      ...
      {process.env.NODE_ENV === "development" && isMobile && (
        <div className="absolute bottom-4 right-4 z-30 bg-black bg-opacity-75 text-white p-2 rounded text-xs">
